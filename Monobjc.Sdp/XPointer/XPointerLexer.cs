...
namespace Mvp.Xml.XPointer
{
	...
	internal class XPointerLexer
	{
		...
		{
			if (p == null)
				throw new ArgumentNullException("pointer", Monobjc.Tools.Sdp.Properties.Resources.NullXPointer);
			_ptr = p;
			NextChar();
		}
			...
			switch (_currChar)
			{
				...
				case '^':
					...
					if (_currChar == '^' || _currChar == '(' || _currChar == ')')
					{
						...
						throw new XPointerSyntaxException(Monobjc.Tools.Sdp.Properties.Resources.CircumflexCharMustBeEscaped);
					break;
				default:
					...
					else if (LexUtils.IsStartNameChar(_currChar))
					{
						...
						if (_currChar == ':')
						{
							...
							if (LexUtils.IsStartNCNameChar(_currChar))
								...
								throw new XPointerSyntaxException(String.Format(CultureInfo.CurrentCulture, Monobjc.Tools.Sdp.Properties.Resources.InvalidNameToken, _prefix, _currChar));
						}
		...
		public string ParseEscapedData()
		{
			...
			while (true)
			{
				switch (_currChar)
				{
					case '^':
						if (!NextChar())
							throw new XPointerSyntaxException(Monobjc.Tools.Sdp.Properties.Resources.UnexpectedEndOfSchemeData);
