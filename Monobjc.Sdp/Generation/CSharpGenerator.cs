...
namespace Monobjc.Tools.Sdp.Generation
{
    public class CSharpGenerator : Generator
    {
        ...
        {
            builder.AppendLine("//");
            builder.AppendLine("// This file was generated by the Monobjc SDEF tool");
            builder.AppendLine("//");
        ...
        {
            String className = NamingHelper.GenerateDotNetName(context.Prefix, cls.name);
            String baseClassName = String.IsNullOrEmpty(cls.inherits) ? "SBObject" : NamingHelper.GenerateDotNetName(context.Prefix, cls.inherits);

            IEnumerable<element> elements = cls.element ?? new element[0];
            IEnumerable<property> properties = cls.property ?? new property[0];
            ...
            foreach (element element in elements.OrderBy(e => e.type))
            {
                // TODO: Add doc
                String elementName;
            ...
            foreach (property property in properties.OrderBy(p => p.name))
            {
                String propertyType = context.ConvertType(property.type);
            ...
            foreach (command command in context.Commands.OrderBy(c => c.name))
            {
                String returnType = command.result != null ? context.ConvertType(command.result.type1) : "void";

                String methodName = NamingHelper.GenerateDotNetName(String.Empty, command.name);
                if (command.parameter != null)
                {
                    methodName = command.parameter.Aggregate(methodName, (name, p) => NamingHelper.GenerateDotNetName(String.Empty, p.name));
                }
                // TODO: Add doc
                ...
                if (command.parameter != null)
                {
                    ...
                    {
                        parameters.Add(context.ConvertType(parameter.type1) + " " + parameter.name);
                    }
                }
