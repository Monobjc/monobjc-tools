...
/*
 * Based on the source code from MindTouch and Microsoft.
 * 
...
namespace Monobjc.Tools.Generator.Parsers.Sgml
{
    ...
    internal class HWStack
    {
        ...
        {
            get { return (i >= 0 && i < this.m_size) ? this.m_items[i] : null; }
        ...
        public object Pop()
        {
            ...
            if (this.m_count > 0)
            {
                return this.m_items[this.m_count - 1];
            }
        ...
        public object Push()
        {
            ...
            return this.m_items[this.m_count++];
        }
...
/*            set
        ...
        {
            get { return (this.m_literalValue == null); }
        }
    }
    ...
    public class SgmlReader : XmlReader
    {
        ...
        [SuppressMessage("Microsoft.Naming", "CA1705",
            Justification = "SgmlReader's standards for constants are different to Microsoft's and in line with older C++ style constants naming conventions.  Visually, constants using this style are more easily identifiable as such.")]
        ...
        public SgmlDtd Dtd
        {
            get
            {
                if (this.m_dtd == null)
                {
                    this.LazyLoadDtd(this.m_baseUri);
                }

                return this.m_dtd;
            }
            set { this.m_dtd = value; }
        }
        ...
        {
            get { return this.m_rootElementName; }
        }
        ...
        {
            get { return this.m_stripDocType; }
        ...
        {
            get { return this.m_ignoreDtd; }
            set { this.m_ignoreDtd = value; }
        }
        ...
        {
            get { return this.m_errorLogFile; }
            set
            {
        ...
        private void Log(string msg, params string[] args)
        {
            if (this.ErrorLog != null)
            {
                ...
                else
                {
                    ...
                    this.ErrorLog.WriteLine("### Error in {0}#{1}, line {2}, position {3}: {4}", path, this.m_current.Name, this.m_current.Line, this.m_current.LinePosition, err);
                }
            }
        }
                    ...
                    default:
                        {
                            ...
                            if (!this.unknownNamespaces.TryGetValue(prefix, out value))
                            {
                                if (this.unknownNamespaces.Count > 0)
                                {
                                    value = UNDEFINED_NAMESPACE + this.unknownNamespaces.Count;
                                }
                                else
                                {
                                    value = UNDEFINED_NAMESPACE;
                                }
                                this.unknownNamespaces[prefix] = value;
                            }
                            return value;
                        }
                }
        ...
        {
            get { return this.m_baseUri == null ? "" : this.m_baseUri.AbsoluteUri; }
        }
        ...
        {
            get { return this.m_whitespaceHandling; }
        ...
        {
            get { return this.GetAttribute(name, namespaceURI); }
        }
        ...
        public override bool MoveToNextAttribute()
        {
            ...
            {
                this.MoveToAttribute(this.m_apos + 1);
                return true;
            }
            else
            {
        ...
        private bool ParseTag(char ch)
        {
            ...
            else if (ch == '!')
            {
                ...
                {
                    string name = this.m_current.ScanToken(this.m_sb, declterm, false);
        ...
        {
            string name = null;
            if (this.m_state != State.PseudoStartTag)
            {
            ...
            while (ch != Entity.EOF && ch != '>')
            {
                ...
                string aname = this.ScanName(aterm);
                ch = this.m_current.SkipWhitespace();
                ...
                if (ch == '=' || ch == '"' || ch == '\'')
                {
                    ...
                    {
                        quote = ch;
                        value = this.ScanLiteral(this.m_sb, ch);
                    }
                    else if (ch != '>')
                    {
                        string term = avterm;
        ...
        {
            char ch = this.m_current.ReadChar(); // skip '['
            ch = this.m_current.SkipWhitespace();
            string name = this.m_current.ScanToken(this.m_sb, cdataterm, false);
            if (name.StartsWith("if "))
            {
                // 'downlevel-revealed' comment (another atrocity of the IE team)
                this.m_current.ScanToEnd(null, "CDATA", ">");
                return false;
            }
            ...
            else
            {
                ...
                string value = this.m_current.ScanToEnd(this.m_sb, "CDATA", "]]>");
        ...
        private void ParseDocType()
        {
            ...
            if (ch != '>')
            {
                ...
                if (ch != '[')
                {
                    string token = this.m_current.ScanToken(this.m_sb, dtterm, false);
                ...
                this.LazyLoadDtd(this.m_current.ResolvedUri);
            }
        ...
        {
            string name = this.m_current.ScanToken(this.m_sb, piterm, false);
            string value = null;
            ...
            {
                this.Push(name, XmlNodeType.ProcessingInstruction, value);
                return true;
            }
            ...
            while (ch != Entity.EOF)
            {
                if (ch == '<')
                {
                    ...
                    else if (ch == '/')
                    {
                        ...
                        if (this.ParseEndTag() && string.Equals(this.m_endTag, this.m_node.Name, StringComparison.OrdinalIgnoreCase))
                        {
        ...
        {
            get { return this.m_state == State.Eof; }
        }
