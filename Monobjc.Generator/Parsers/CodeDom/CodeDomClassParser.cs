...
namespace Monobjc.Tools.Generator.Parsers.CodeDom
{
    public class CodeDomClassParser : CodeDomTypeParser
    {
        ...
        public CodeDomClassParser(NameValueCollection settings, TypeManager typeManager) : base(settings, typeManager) {}
        ...
        private MethodEntity ExtractMethod(MethodDeclaration methodDeclaration)
        {
            ...
            if (signatureEnumerator.MoveNext())
            {
                ...
                methodEntity.ReturnType = this.TypeManager.ConvertType(signatureEnumerator.Current.TrimAll(), out isOut, out isByRef, out isBlock);
            }
            else
            {
        ...
        private PropertyEntity ExtractProperty(PropertyDeclaration propertyDeclaration)
        {
            ...
            {
                bool isOut, isByRef, isBlock;
                propertyEntity.Type = this.TypeManager.ConvertType(signatureEnumerator.Current.TrimAll(), out isOut, out isByRef, out isBlock);
            }
            else
            {
